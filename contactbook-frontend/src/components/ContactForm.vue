...
export default {
  ...
  methods: {
    ...
    deleteContact() {
      this.$emit("delete:contact", this.contactLocal._id); // Đã sửa .id -> ._id
    },
    Cancel() {
      const reply = window.confirm('Quay lại mà chưa lưu các thay đổi?'); // Dịch thông báo sang tiếng Việt
      if (!reply) {
        return false;
      } else {
